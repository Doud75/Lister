...
test.describe('Default Band', () => {
    ...
    test('clicking star on a non-default band makes it the default', async ({ page }) => {
        ...
        await expect(page.getByRole('button', { name: 'Groupe par défaut' }).first()).toBeVisible();

        // Cliquer sur l'étoile vide de bandB
        await page.getByRole('button', { name: `Définir ${bandB} comme groupe par défaut` }).click();
        await page.waitForURL('/dashboard');

        // Maintenant bandB a l'étoile pleine
        await expect(page.getByRole('button', { name: 'Groupe par défaut' })).toBeVisible();
        // L'étoile vide doit exister pour bandA
        await expect(page.getByRole('button', { name: `Définir ${bandA} comme groupe par défaut` })).toBeVisible();
    });
    ...
    test('after login, active band is the default band', async ({ page }) => {
        ...
        const defaultBtn = page.getByRole('button', { name: 'Groupe par défaut' });
