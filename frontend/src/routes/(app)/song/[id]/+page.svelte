...
<div class="container mx-auto max-w-3xl px-4 sm:px-6">
    <header class="mb-8">
        ...
        <div class="mt-5">
            <a
                href={editHref}
                class="inline-flex items-center gap-2 rounded-md bg-indigo-600 px-4 py-2 text-sm font-semibold text-white shadow-sm transition-colors hover:bg-indigo-500"
            >
    ...
    <div class="rounded-xl bg-white p-6 shadow-lg dark:bg-slate-800">
        ...
        {#if song.lyrics?.Valid && song.lyrics.String}
            <p class="whitespace-pre-wrap text-slate-800 dark:text-slate-100 leading-relaxed">
                {song.lyrics.String}
            </p>
        {:else}
